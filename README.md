...
Проведена приоритизация гипотез по фреймворкам ICE и RICE. После анализа результатов A/B-теста принято решение об окончании его проведения, так как конверсия тестовой группы увеличилась

Библиотеки:
**`pandas`**, **`datetime`**, **`numpy`**, **`matplotlib.pyplot`**, **`scipy.stats`**, **`seaborn`**
...
Проведен анализ рынка для подготовки презентации инвесторам - будет ли успешным и популярным на долгое время кафе, в котором гостей обслуживают роботы-официанты. Проведена предобработка данных. Осуществлен запрос к API Яндекс.Геокодер для парсинга района, в котором расположены кафе-конкуренты. Были проанализированы их характеристики.
...
Библиотеки:
**`pandas`**, **`numpy`**, **`matplotlib.pyplot`**, **`plotly`**, **`seaborn`**, **`re`**, **`requests`**, **`bs4`**
